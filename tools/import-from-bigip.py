...
class VarTreeDumper(yaml.Dumper):
    """YAML dumper for var tree output.

    Purpose:
        Custom Dumper that properly indents nested structures, ensuring
        the generated YAML files match the repository's formatting conventions.

    Example:
        yaml.dump(data, Dumper=VarTreeDumper)
    """
...
def str_representer(dumper: yaml.Dumper, data: str) -> yaml.Node:
    """YAML representer for strings with proper formatting.

    Purpose:
        Ensures multiline strings use the literal block style (|) while
        single-line strings use plain style for cleaner output.
    ...
    Outputs:
        yaml.Node: A scalar node with appropriate style.
    """
    if "\n" in data:
...
class BigIPConnection:
    ...
    Purpose:
        ...
        helpers to query the BIG-IP REST API for object imports.
    ...
    def get_all(self, uri: str) -> list[dict[str, Any]]:
        """Paginate through a BIG-IP REST endpoint and return all items.

        Inputs:
            uri (str): API endpoint path (e.g., "ltm/pool").

        Returns:
            list[dict]: All items from the paginated response.
        """
...
class Importer:
    ...
    def _import_waf_server_technologies(self, spec: ImportSpec) -> int:
        """Import WAF server technologies from BIG-IP to YAML.

        Inputs:
            spec (ImportSpec): The import specification for WAF technologies.

        Returns:
            int: Number of technologies imported.

        Side effects:
            For each ASM policy, queries its server technology reference and
            extracts technology names into the output YAML.
        """
    ...
    def _import_apm_sso_configs(self, spec: ImportSpec) -> int:
        """Import APM SSO configurations from BIG-IP to YAML.

        Inputs:
            spec (ImportSpec): The import specification for APM SSO configs.

        Returns:
            int: Number of SSO configs imported.

        Side effects:
            Transforms each SSO config and writes import-{obj_type}.yml.
        """
        items = self.conn.get_all(spec.endpoint)
        if not items:
            return 0
    ...
    def _import_apm_policy_nodes(self, spec: ImportSpec) -> int:
        """Import APM policy nodes from BIG-IP to YAML.

        Inputs:
            spec (ImportSpec): The import specification for APM policy nodes.

        Returns:
            int: Number of policy nodes imported.

        Side effects:
            For each APM policy, extracts its access policy nodes
            and writes them to the output YAML.
        """
        policies = self.conn.get_all(spec.endpoint)
        if not policies:
            return 0
