...
from filter_plugins.bigip_var_filters import compile_gtm_application_intent
...
def validate_wide_ips(validator, wide_ips: list, state: dict[str, object]) -> None:
    ...
    for obj in wide_ips:
        ...
        if wide_ip_pools is None or not isinstance(wide_ip_pools, list) or not wide_ip_pools:
            validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` must define a non-empty `pools` list")
            continue
        for pool_index, pool in enumerate(wide_ip_pools):
            if not isinstance(pool, dict):
                validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` pool binding {pool_index} must be a mapping")
                continue
            pool_mode = pool.get("pool_mode")
            if pool_mode not in {"inline", "reference"}:
                validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` pool binding {pool_index} must define `pool_mode: inline` or `pool_mode: reference`")
                continue
            if pool_mode == "reference":
                pool_ref = pool.get("pool_ref")
                if not isinstance(pool_ref, str) or not pool_ref:
                    validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` reference pool binding {pool_index} must define string `pool_ref`")
                unsupported = sorted(set(pool) - {"pool_mode", "pool_ref", "ratio", "partition"})
                if unsupported:
                    validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` reference pool binding `{pool_ref}` contains unsupported keys: {', '.join(unsupported)}")
                continue

            inline_pool = pool.get("pool")
            if not isinstance(inline_pool, dict):
                validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool binding {pool_index} must define mapping `pool`")
                continue
            if "name" not in inline_pool:
                validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool binding {pool_index} must define `pool.name`")
                continue
            unsupported = sorted(set(pool) - {"pool_mode", "pool", "ratio", "partition"})
            if unsupported:
                validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool binding `{inline_pool.get('name')}` contains unsupported keys: {', '.join(unsupported)}")
            members = inline_pool.get("members")
            if not isinstance(members, list) or not members:
                validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool `{inline_pool.get('name')}` must define a non-empty `members` list")
                continue
            for member_index, member in enumerate(members):
                if not isinstance(member, dict):
                    validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool `{inline_pool.get('name')}` member {member_index} must be a mapping")
                    continue
                server_mode = member.get("server_mode")
                if server_mode not in {"inline", "reference"}:
                    validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool `{inline_pool.get('name')}` member {member_index} must define `server_mode: inline` or `server_mode: reference`")
                    continue
                if server_mode == "reference":
                    server_reference = member.get("server")
                    if not isinstance(server_reference, str) or not server_reference:
                        validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool `{inline_pool.get('name')}` member {member_index} must define string `server` when `server_mode` is `reference`")
                    continue
                server_payload = member.get("server")
                if not isinstance(server_payload, dict):
                    validator.error(obj.relpath, f"GTM application intent `{obj.data.get('name')}` inline pool `{inline_pool.get('name')}` member {member_index} must define mapping `server` when `server_mode` is `inline`")
                    continue
                validator.require_fields(server_payload, ["name"], obj.relpath, f"GTM inline server for application intent `{obj.data.get('name')}` member {member_index}")
                datacenter_mode = server_payload.get("datacenter_mode")
                if datacenter_mode not in {"inline", "reference"}:
                    validator.error(obj.relpath, f"GTM inline server `{server_payload.get('name')}` must define `datacenter_mode: inline` or `datacenter_mode: reference`")
                    continue
        ...
        compiled_service = compile_gtm_application_intent(
            obj.data,
    ...
    validator.check_duplicates(
        wide_ips,
        lambda obj: ("gtm_application_intent", obj.partition, obj.data.get("name"), obj.data.get("record_type", "a")),
        "GTM application intent",
    )
...
def _validate_compiled_intents(validator, compiled_gtm_datacenters, compiled_gtm_servers, compiled_gtm_pools, compiled_gtm_wide_ips, state) -> None:
    ...
    for source_obj, datacenter in compiled_gtm_datacenters:
        ...
        if datacenter_name in (None, ""):
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiled an unnamed datacenter")
            continue
        ...
        if fq_datacenter_name in state["canonical_datacenter_names"]:
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiles datacenter `{fq_datacenter_name}` that already exists in canonical `gtm_datacenters` trees")
            continue
        if fq_datacenter_name in compiled_datacenter_names:
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiles duplicate datacenter `{fq_datacenter_name}`")
            continue
    ...
    for source_obj, server in compiled_gtm_servers:
        ...
        if server_name in (None, ""):
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiled an unnamed server")
            continue
        ...
        if fq_server_name in state["canonical_server_names"] or fq_server_name in compiled_server_names:
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiles duplicate server `{fq_server_name}`")
            continue
    ...
    for source_obj, normalized_pool in compiled_gtm_pools:
        ...
        if pool_name in (None, ""):
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiled an unnamed pool")
            continue
        ...
        if fq_pool_name in state["canonical_pool_names"] or fq_pool_name in compiled_pool_names:
            validator.error(source_obj.relpath, f"GTM application intent `{source_obj.data.get('name')}` compiles duplicate pool `{fq_pool_name}`")
            continue
    ...
    for source_obj, compiled_wide_ip in compiled_gtm_wide_ips:
        ...
        for pool in compiled_wide_ip.get("pools", []) or []:
            ...
            if pool_ref not in state["active_pool_names"]:
                validator.error(source_obj.relpath, f"GTM application intent `{compiled_wide_ip.get('name')}` references undefined pool `{pool_ref}`")
