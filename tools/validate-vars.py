...
class AnsibleVarLoader(yaml.SafeLoader):
    ...
    Usage:
        ...
        values or other custom tags. The loader delegates to the parent constructor
        for standard YAML structures.

    Example:
        data = yaml.load(open("vars/common.yml"), Loader=AnsibleVarLoader)
    """
    pass
...
class TreeSpec:
    """Specification for a var tree: where to find YAML files and how to parse them.

    Purpose:
        Defines the mapping between a domain object type (e.g., "ltm_pools")
        and its on-disk location, YAML top-level key, and settings keys.

    Attributes:
        name (str): Unique identifier for this tree (e.g., "ltm_pools").
        active_dir (Path): Directory containing active object definitions.
        deletion_dir (Path): Directory containing deletion markers.
        top_key (str): The YAML top-level key that holds the object list (e.g., "ltm_pools").
        settings_key (str): The settings.yml key for defaults (e.g., "ltm_pool_defaults").
        extra_settings_keys (tuple): Additional settings keys to load (e.g., monitor sets).
    """
    name: str
    active_dir: Path
    deletion_dir: Path
...
@dataclass
class LoadedObject:
    """A loaded object from a var tree, enriched with metadata.

    Purpose:
        Wraps a single declared object with its source file, defaults, and
        whether it came from the deletions tree.

    Attributes:
        tree (TreeSpec): The tree specification this object belongs to.
        source_file (Path): The YAML file this object was loaded from.
        defaults (dict): Merged directory-level defaults for this object.
        data (dict): The actual object data from the YAML file.
        from_deletions (bool): True if loaded from a deletions/ directory.

    Properties:
        relpath (str): Relative path for error messages.
        effective_state (str): "absent" if from deletions, otherwise data["state"] or "present".
        partition (str): The object's partition, falling back to defaults then "Common".
    """
    tree: TreeSpec
    source_file: Path
...
class Validator:
    ...
    def validate_tls(self) -> None:
        ...
        Side effects:
            Adds validation errors to self.errors for any violations.
        """
    ...
    def validate_ltm(self) -> None:
        """Validate LTM domain objects: nodes, profiles, pools, monitors, virtual servers, persistence, iRules, data groups, policies.
        ...
        Validates:
            - Nodes: name required; address or FQDN required; duplicate detection.
            - Monitors: name and type required; type must be supported.
            - Profiles: name and type required; parent profile validation.
            - Pools: name required; monitor validation; member references.
            - Virtual servers: name, destination, pool references.
            - Persistence profiles, iRules, data groups, policies: structure and references.
            - RKE2 server intents: cluster intent compilation and validation.
        """
        ...
        rke2_server_intents = self.objects.get("ltm_rke2_server_intents", [])
        ...
        for obj in pools:
            ...
            settings_payload = self.load_settings_hierarchy_payload(obj.source_file, VARS_DIR / "ltm" / "pools")
        ...
        self.check_duplicates(
            rke2_server_intents,
            lambda obj: ("ltm_rke2_server_intent", obj.partition, obj.data.get("name")),
        ...
        for source_obj, normalized_pool in compiled_rke2_pools:
            ...
            monitors_list = normalized_pool.get("monitors", [])
            if monitors_list is not None and not isinstance(monitors_list, list):
                self.error(source_obj.relpath, f"RKE2 compiled pool `{pool_name}` `monitors` must be a list")
            else:
                for monitor in monitors_list or []:
                    self.validate_monitor_reference(
            ...
            self.validate_ltm_pool_members(source_obj.relpath, pool_name, pool_partition, normalized_pool.get("members"), active_node_names)
        ...
        for obj in virtual_servers:
            ...
            settings_payload = self.load_settings_hierarchy_payload(obj.source_file, VARS_DIR / "ltm" / "virtual_servers")
        ...
        for source_obj, virtual_server in compiled_rke2_virtual_servers:
            ...
            if virtual_server.get("destination") in (None, "") or virtual_server.get("destination_port") in (None, ""):
                self.error(source_obj.relpath, f"RKE2 compiled virtual server `{name}` must define `destination` and `destination_port`")
                continue
            pool_ref = self.normalize_pool_reference(virtual_server.get("pool"), vs_partition)
            if pool_ref not in active_pool_names:
                self.error(source_obj.relpath, f"RKE2 compiled virtual server `{name}` references undefined pool `{pool_ref}`")
            profiles = virtual_server.get("profiles")
            if profiles is not None:
                if not isinstance(profiles, list):
                    self.error(source_obj.relpath, f"RKE2 compiled virtual server `{name}` `profiles` must be a list")
                else:
                    for profile_reference in profiles:
                        self.validate_profile_reference(
                            source=source_obj.relpath,
            ...
            vs_vlans = virtual_server.get("vlans")
            if vs_vlans is not None:
                if not isinstance(vs_vlans, list):
                    self.error(source_obj.relpath, f"RKE2 compiled virtual server `{name}` `vlans` must be a list")
                else:
                    for vlan in vs_vlans:
                        self.validate_named_or_fq_reference(
                            source_obj.relpath,
    ...
    def validate_gtm(self) -> None:
        """Validate GTM domain objects: datacenters, servers, pools, wide IPs, topology regions/records, monitors.

        Purpose:
            Checks structural correctness and cross-object references for all
            GTM (Global Traffic Manager) objects.

        Validates:
            - Datacenters: name required; location/contact fields.
            - Servers: name, datacenter reference, virtual server references.
            - Pools: name, lb_method, member references to LTM virtual servers.
            - Wide IPs: name, record_type, pool references.
            - Topology regions: name, region_members.
            - Topology records: source, destination, weight.
            - Monitors: name and type validation.
        """
        ...
        for obj in pools:
            ...
            settings_payload = self.load_settings_hierarchy_payload(obj.source_file, VARS_DIR / "gtm" / "pools")
...
def main() -> int:
    """CLI entry point for validate-vars.

    Purpose:
        Validates all var trees in the repository against the field model.
        Loads all objects, runs domain-specific validation, and reports errors.
